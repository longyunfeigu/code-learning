...
- **No external system dependencies (HTTP, SDK, cache)**
...
- `application/services/user_service.py` - Orchestration: Redis locks, DTO conversion
...
- Sensitive fields auto-redacted: `password`, `secret`, `api_key`, etc.
- Limited to first 2048 bytes (configurable via `LOG_REQUEST_BODY_MAX_BYTES`)
- multipart/form-data not logged by default

## User Management

User registration, authentication, and profile management with password validation and security.
...
**Anonymous RPCs**: `Register`, `Health`
**RPCs**: all others are available without authentication
