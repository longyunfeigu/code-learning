...
- 全局配置：`core/config.py`（Pydantic Settings v2；支持嵌套环境变量 `__`）
  - 强制 `SECRET_KEY`：`core/config.py:158` 未设置将直接抛错，确保应用安全
...
- 请求日志：`api/middleware/logging.py`
  ...
  - 敏感字段自动脱敏：`password`、`secret`、`api_key` 等
...
## API 层（表现层）
- 应用服务依赖：`api/dependencies.py`
- 路由：
  - 用户：`api/routes/user.py`（注册/更新/改密/列表/激活/停用/删除）
...
4) 练手：新增一个简单实体 + 列表/创建接口，按"配方"完成端到端闭环
